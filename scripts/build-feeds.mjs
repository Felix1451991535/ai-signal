...
const FEEDS = [
  ...
  { id: "naturebme", url: "https://www.nature.com/natbiomedeng.rss", cap: 20 },
  { id: "sdbme", url: "https://www.sciencedaily.com/rss/health_medicine/medical_devices.xml", cap: 20 },
  { id: "embs", url: "https://www.embs.org/feed/", cap: 20 },
  { id: "mithealth", url: "https://news.mit.edu/rss/topic/health", cap: 20 }
];
...
function normalize(id, items, cap) {
  ...
  for (const raw of items) {
    ...
    let time = Date.parse(raw.pubDate || raw.published || raw.updated || raw.date || raw["dc:date"] || "");
...
async function load(feed) {
  ...
  // RDF/RSS 1.0（如 Nature）：根元素为 rdf:RDF，item 为其直接子级
  if (j["rdf:RDF"]) return parseRssItems(j["rdf:RDF"]);
